...
document.addEventListener('DOMContentLoaded', () => {
    const initialize = async () => {
        ...
        const loadModel = async (itemName, category) => {
            ...
            if (!models[itemName]) {
                if (!loadingModels[itemName]) {
                    loadingModels[itemName] = loadGLTF(modelPath).then(model => {
                        normalizeModel(model.scene, itemHeights[itemName]);
                        models[itemName] = model.scene; // Cache loaded model
                        delete loadingModels[itemName]; // Remove from loading cache
                    });
                }
                await loadingModels[itemName]; // Wait for the model to load
            }
        ...
        document.addEventListener('click', (event) => {
            ...
            if (intersects.length > 0) {
                ...
                currentInteractedItem.scale.multiplyScalar(1.05); // Scale up to indicate selection
            }
        });
